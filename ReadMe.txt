How to run News Site Project

1. Download the the zip file
2. Extract the file and copy News Site folder
3.Paste inside root directory(for xampp xamp/htdocs, for wampp/www,fpr lamp var/www/html)
4. Create a database with name "news"
5. Import newsportal.sql file(given inside the zip package in sql file folder)
6.run the script http://localhost/stock news (frontend)
7. For admin panel  http://localhost/stock news/admin

credential for admin panel :
username / email : stock or m.zubair9824(use any one) 
Password : thestock
